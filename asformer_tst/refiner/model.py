...
class Asformer(nn.Module):
    def __init__(self, num_decoders, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate=0.3):
...
class ourmodel(nn.Module):
    def __init__(self, args, backbone, n_features, n_classes, n_stages, n_layers, n_stages_asb, n_stages_brb, **kwargs):
        ...
        self.pd_linear2 = MLP(64, 256, 512, 3)
        ...
        self.sd_linear2 = MLP(n_classes, 128, self.sd_dim, 3)
        self.label_embedding = nn.Embedding(n_classes, self.sd_dim)

        #######################################################
        # >>>>>>>>>> local cross-attention & global self-attention <<<<<<<<<<<<<<<<
        #######################################################
        self.local_ca = CrossAttention(self.sd_dim*2, 1, self.sd_dim*2, 0.1, 'relu')
        self.local_ca2 = CrossAttention(self.sd_dim*2, 1, self.sd_dim*2, 0.1, 'relu')

        self.local_sa = SelfAttention(self.sd_dim*2, nhead=1, dim_feedforward=self.sd_dim*2, dropout = 0.1)
        self.local_sa2 = SelfAttention(self.sd_dim*2, nhead=1, dim_feedforward=self.sd_dim*2, dropout = 0.1)

        self.rate = 4
        print("*************head: localsa_rate global", 1)

        self.class_embed = nn.Linear(self.sd_dim*2, n_classes+1)
        # self.class_embed = nn.Linear(self.sd_dim*2, n_classes)
        self.mask_embed = nn.Linear(self.sd_dim*2, n_features)


    def forward(self, action_idx=None, x=None, batch_target=None, batch_targets_segment=None, location_segment=None):
        # action_idx: [T]. x: [bs, bk_dim, T]. batch_target: [bs, T]

        #######################################################
        # >>>>>>>>>> frame decoder <<<<<<<<<<<<<<<<
        #######################################################
        out_cls, bk_f = self.backbone(x)
        if self.training:
            action_idx = action_idx
        else:
            action_idx = torch.max(out_cls[-1][0], 0)[1].detach()
            print(111)
        ...
        pd_f_9 = pd_f_all[-2]
        pd_f_10 = pd_f_all[-1]
        pd_f_memory_8 = self.pd_linear1(pd_f_8.permute(0, 2, 1)).permute(0, 2, 1) # [bs, sd_dim*2, T]
        pd_f_memory_9 = self.pd_linear2(pd_f_9.permute(0, 2, 1)).permute(0, 2, 1) # [bs, sd_dim*2, T]
        ...
        pd_f_memory_8 = pd_f_memory_8.permute(2, 0, 1) # [T, bs, sd_dim*2]
        T, _, input_dim = pd_f_memory_8.shape
        pos_ca = get_sinusoid_encoding_table(T, input_dim).to(bk_f.device).unsqueeze(1) # [T, 1, sd_dim*2]

        # >>>> query(segment decoder)
        tgt = torch.zeros_like(segment_feat).to(bk_f.device) # [num_seg, bs, sd_dim*2]
        tgt = self.local_ca(tgt=tgt, query_pos=segment_feat, memory=pd_f_memory_8, pos=pos_ca, action_idx=action_idx).permute(1, 0, 2)  # [num_seg, bs, sd_dim*2] => [bs, num_seg, sd_dim*2]
        ...
        pos_sa = get_sinusoid_encoding_table(num_seg, hidden_dim).to(bk_f.device) # [num_seg, sd_dim*2]
        refine_pred1 = self.local_sa(src=tgt, src_mask=None, src_key_padding_mask=None, pos=pos_sa, rate=self.rate) # [bs, num_seg, sd_dim*2]

        
        #######################################################
        # >>>>>>>>>> local cross-attention <<<<<<<<<<<<<<<<
        #######################################################
        pd_f_memory_9 = pd_f_memory_9.permute(2, 0, 1) # [T, bs, sd_dim*2]
        refine_pred2 = self.local_ca2(tgt=refine_pred1.permute(1, 0, 2), query_pos=segment_feat, memory=pd_f_memory_9, pos=pos_ca, action_idx=action_idx).permute(1, 0, 2)  # [num_seg, bs, sd_dim*2] => [bs, num_seg, sd_dim*2]

        # >>>>>>a2: tgt seg embed: + start_end
        
        #######################################################
        # >>>>>>>>>> readout <<<<<<<<<<<<<<<<
        #######################################################
        refine_pred = torch.cat((refine_pred1.unsqueeze(0), refine_pred2.unsqueeze(0))) # [num_decoder, bs, num_seg, se_dim*2]
        segment_cls = self.class_embed(refine_pred) # [num_decoder, bs, num_seg, num_class]
        mask_embed = self.mask_embed(refine_pred) # [num_decoder, bs, num_seg, bk_dim] x [bs, bk_dim, T]

        segment_mask = torch.einsum('nbsd,bdt->nbst', mask_embed, pd_f_10) # [num_decoder, bs, num_seg, T]
        return segment_cls, segment_mask, action_idx, GTmask_list
