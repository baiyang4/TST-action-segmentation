...
def load_meta(data_root, model_root, result_root, record_root, dataset, split, model_name):
    ...
    print('Created :'+model_dir, flush=True)
    print('Created :'+result_dir, flush=True)
    print('Created :'+record_dir, flush=True)
...
def eval_txts(data_root, result_dir, dataset, split, name):
    ...
    # print("Acc: %.4f" % (accu), flush=True)
    # print('Edit: %.4f' % (edit), flush=True)
    
    results['accu']=round(accu, 3)
    results['edit']=round(edit, 3)
    ...
    for s in range(len(overlap)):
        ...
        results['F1@%0.2f' % (overlap[s])] = round(f1, 3)
        # print('F1@%0.2f: %.4f' % (overlap[s], f1), flush=True)
    return results
